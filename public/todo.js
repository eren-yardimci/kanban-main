document.addEventListener('DOMContentLoaded', async () => {
  ...
  const taskColorInput = document.querySelector('select[name="color"]'); // Renk seçimi
  ...
  taskForm.addEventListener('submit', async (event) => {
      ...
      const taskColor = taskColorInput.value; // Seçilen renk
      ...
      try {
          const response = await fetch('/tasks', {
              ...
              body: JSON.stringify({
                  ...
                  color: taskColor, // Renk bilgisi ekleniyor
...
function addTaskToLane(task) {
  ...
  // Seçilen renge göre arka plan rengini ayarla
  if (task.color) {
      taskElem.style.backgroundColor = task.color;
  }
